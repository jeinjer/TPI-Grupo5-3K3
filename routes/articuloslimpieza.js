...
router.get('/api/articuloslimpieza', async function (req, res, next) {
  let = data = await db.articuloslimpieza.findAll({
    attibutes: ["IdArticuloLimpieza", "Nombre"],
    where:{Activo: true},
  });
  res.json(data);
});
...
router.get('/api/articuloslimpieza/:id', async function (req, res,next) {
    let data = await db.articuloslimpieza.findByPk(req.params.id, {
      attributes: ["IdArticuloLimpieza", "Nombre", "Precio", "Stock", "FechaEnvasado"],
      where: {Activo: true},
    });
   if (data){
    res.json(data);
...
router.put('/api/articuloslimpieza/:id', async (req, res) => {
    ...
    try {
      ...
      if (articuloLimpieza){
        ...
        articuloLimpieza.Nombre = Precio;
        articuloLimpieza.Nombre = Stock;
        articuloLimpieza.Nombre = FechaEnvasado;
  ...
  router.delete('/api/articuloslimpieza/:id', async (req, res) => {
    ...
    if (articuloLimpieza){
      ...
      await articuloLimpieza.save();
      res.json({ message: 'articulo de limpieza marcado como inactivo' });
    } else {
